        ...
        node_(node)
{
    ...
    enableUpdateSubscriber_ = node_->create_subscription<exo_msgs::msg::DevToggle>("dev_toggles",1, std::bind(&X2MachineROS2::enablerCallback, this, _1));
...
void X2MachineROS2::enablerCallback(const exo_msgs::msg::DevToggle::SharedPtr enable) {
    if (enable->pd == 1) {
        x2FollowerState_->posReader.start();
    } else {
        x2FollowerState_->posReader.stop();
    }
}
...
void X2MachineROS2::gaitParamCallback(const exo_msgs::msg::GaitParameter::SharedPtr gaitParam){
    x2FollowerState_->posReader.setPeriod(gaitParam->step_period);
}
